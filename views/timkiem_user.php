...
<?php if (isset($_SESSION['tennguoidung'])and($_SESSION['quyennguoidung'])!=1 || isset($_SESSION['tennguoidung'])and($_SESSION['quyennguoidung']) == null ) {
  ...
  <?php include('includes/admin/hearder.php') ?>    
          <div class="nd">
            ...
            <div class="boxtk"><input id="tk" type="Search" name="ten" placeholder=" Tìm Kiếm..."/> </div> </form><
              ...
              <tbody>
                <?php
                  if ($data_tk==0) {
                  ?>
                  <h1 style="text-align: center; font-size: 20px;"><?php echo "Không tìm thấy kết quả với ".$ten; ?></h1> 
                  <?php  
                  }
                  else{
                  ?>  
                  <h2 style="text-align: center; font-size: 20px;"><?php echo "Kết quả với từ ".$ten; ?> với <?php echo count($data_tk); ?> kết quả trả về </h2>
                <?php 
                  $i=1;
